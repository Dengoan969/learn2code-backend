...
def main():
    ...
    if "error" in code:
        ...
        # Normal response with new format
        result = {
            "success": True,
            "finalState": {
                "sprites": [
            ...
            "trace": {
                "events": [
                    {"step": 1, "eventType": "move", "details": {"cells": 2}},
                    {"step": 2, "eventType": "turn", "details": {"degrees": 90, "direction": "right"}}
                ]
            }
        }
...
""");
        ...
        if (_pythonPath == null)
        {
            Assert.Inconclusive("Python not found on system. Skipping sandbox tests.");
        }
    }
    ...
    public async Task ExecuteAsync_WithValidCode_ReturnsSuccessPayload()
    {
        ...
        Assert.That(catSprite.GridX, Is.EqualTo(5));
        Assert.That(catSprite.GridY, Is.EqualTo(3));
